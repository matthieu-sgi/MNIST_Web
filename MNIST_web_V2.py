...
if __name__ == "__main__":
    MNIST_TEST_LEN = len(MNIST("/tmp/mnist", download=True, train=False, transform=Compose([ToTensor(), Lambda(lambda x: x.reshape(-1))])))
    test_valid_frac = 0.5
    ...
    sets = {
        ...
        **{s: d for s, d in zip([Split.VALID, Split.TEST], random_split(MNIST("/tmp/mnist", download=True, train=False, transform=Compose([ToTensor(), Lambda(lambda x: x.reshape(-1))])), lengths=[int(MNIST_TEST_LEN * test_valid_frac), int(MNIST_TEST_LEN * (1 - test_valid_frac))]))},
    }
    ...
    # ==== UINT8 QUANTIZATION
    q_model = model.quantize(delete=[model.i.d, model.h.d], fuse=[["m.i.l", "m.i.a"], ["m.h.l", "m.h.a"]], loader=loaders[Split.TEST])
